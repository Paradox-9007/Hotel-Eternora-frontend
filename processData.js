...
function ExcelDate_to_NormalDate(excelDate) {
  ...
  const normalDate = new Date(
    excelStartDate.getTime() + (excelDate - 1) * 86400000
  );
...
function filter_bookingsData_byArrival(
  bookingsData,
  startDate_inEdate,
  endDate_inEdate
) {
  // get bookings json and return json
...
function filter_bookingsData_byDeparture(
  bookingsData,
  startDate_inEdate,
  endDate_inEdate
) {
...
function generate_Barchart_dashboard_1_inMonth(month, year) {
  ...
  const data = labels.map((day) => {
    ...
    return filter_bookingsData_byArrival(
      bookingsData,
      startDate_inEdate,
      startDate_inEdate
    ).length;
...
function generateHourlyBookingData(dateObj) {
  ...
  dayBookings.forEach((booking) => {
    ...
    if (arrivalDay === targetDay) {
      const arrivalTime = convertEminutesToNormalminute(
        parseInt(booking["Arrival Minute"])
      );
    ...
    if (departureDay === targetDay) {
      const departureTime = convertEminutesToNormalminute(
        parseInt(booking["Departure Minute"])
      );
...
function getMonthlyArrivalCount(bookingsData, month, year) {
  ...
  const monthlyBookings = filter_bookingsData_byArrival(
    bookingsData,
    startDate_inEdate,
    endDate_inEdate
  );
...
function getMonthlyDepartureCount(bookingsData, month, year) {
  ...
  const monthlyBookings = filter_bookingsData_byDeparture(
    bookingsData,
    startDate_inEdate,
    endDate_inEdate
  );
...
function getBookingsForDay(dateObj) {
  const targetDate = new Date(dateObj.year, dateObj.month, dateObj.day);
  const targetDay = NormalDate_to_ExcelDate(targetDate);
  ...
  return bookingsForDay.map((booking) => {
    ...
    return {
      ...
      clientId: booking["Guest ID"],
      time:
        targetDay === parseInt(booking["Arrival Day"])
          ? convertEminutesToNormalminute(parseInt(booking["Arrival Minute"]))
              .time.hours +
            ":" +
            convertEminutesToNormalminute(parseInt(booking["Arrival Minute"]))
              .time.minutes
          : convertEminutesToNormalminute(parseInt(booking["Departure Minute"]))
              .time.hours +
            ":" +
            convertEminutesToNormalminute(parseInt(booking["Departure Minute"]))
              .time.minutes,
      action:
        targetDay === parseInt(booking["Arrival Day"]) ? "Arrive" : "Depart",
    };
  });
}
...
function generateMemberVsGeneralData(year = null, month = null) {
  ...
  let memberData = [];
  let generalData = [];
  let memberNationalities = {};
  let nonMemberNationalities = {};
  ...
  if (!year) {
    ...
    const years = [
      ...new Set(
        bookingsData.map((booking) =>
          new Date(
            ExcelDate_to_NormalDate(booking["Arrival Day"])
          ).getFullYear()
        )
      ),
    ].sort((a, b) => a - b);
    ...
    years.forEach((year) => {
      ...
      const bookings = filter_bookingsData_byArrival(
        bookingsData,
        startDate_inEdate,
        endDate_inEdate
      );
        ...
        if (client) {
          ...
          if (isBookedByMember(booking["Booking ID"])) {
            ...
            memberNationalities[nationality] =
              (memberNationalities[nationality] || 0) + 1;
          } else {
            generalCount++;
            nonMemberNationalities[nationality] =
              (nonMemberNationalities[nationality] || 0) + 1;
          }
        }
      });
    ...
    datasets: [
      {
        ...
        data: memberData,
        backgroundColor: "rgba(75, 192, 192, 0.6)", // Optional: added color for visualization
      },
      {
        label: "General Guest Bookings",
        data: generalData,
        backgroundColor: "rgba(255, 99, 132, 0.6)", // Optional: added color for visualization
      },
    ],
    memberNationalities,
    nonMemberNationalities,
  };
}
...
function memberTierDistribution(year = null, month = null) {
  ...
  if (year && month) {
    ...
    const startDate_inEdate = NormalDate_to_ExcelDate(startDate);
    const endDate_inEdate = NormalDate_to_ExcelDate(endDate);
    filteredBookings = filter_bookingsData_byArrival(
      bookingsData,
      startDate_inEdate,
      endDate_inEdate
    );
  ...
  // Iterate over filtered bookings to count membership tiers
...
function generateProfitComparisonData(year = null, month = null) {
  ...
  datasets[0].data.forEach((memberCount, index) => {
    ...
    const periodBookings = filter_bookingsData_byArrival(
      bookingsData,
      startDate_inEdate,
      endDate_inEdate
    );
    ...
    const memberBookings = periodBookings.filter((booking) =>
      isBookedByMember(booking["Booking ID"])
    );
    const generalBookings = periodBookings.filter(
      (booking) => !isBookedByMember(booking["Booking ID"])
    );
...
function generateUnitPerformanceData(year = null, month = null) {
  ...
  if (!year) {
    ...
    startDate = new Date(
      Math.min(
        ...bookingsData.map(
          (booking) => new Date(ExcelDate_to_NormalDate(booking["Arrival Day"]))
        )
      )
    );
    endDate = new Date(
...
function getAllClientBirthdays(month = null) {
  ...
  const clientBirthdays = clientData.map((client) => {
    ...
    const title = client["Title"] ? `${client["Title"]}. ` : ""; // Add title if it exists, without a dot
    return {
      clientId: client["Client ID"],
      name: `${title}${client["First Name"]} ${client["Surname"]}`, // Construct name with or without title
...
function getBookingsByStatus(bookingsData, startDate, endDate) {
  ...
  // Filter bookings within the date range
  const filteredBookings = filter_bookingsData_byArrival(
    bookingsData,
    startDate_inEdate,
    endDate_inEdate
  );
...
function generateBookingStatusChartData(
  year = null,
  month = null,
  statusLabels = null
) {
      ...
      if (statusIndex !== -1) {
        ...
        bookings.forEach((booking) => {
          ...
          if (year && month) {
            ...
            index = timePoints.findIndex(
              (tp) => tp.getFullYear() === date.getFullYear()
            );
          }
          if (index >= 0) {
...
function generateRevenueChartData(year = null, month = null) {
  if (!year) {
    ...
    return {
      ...
      datasets: [
        {
          label: `Daily Revenue for ${new Date(year, month - 1).toLocaleString(
            "default",
            { month: "long" }
          )} ${year}`,
          data: dailyData,
